...
df_size_edu %>% 
  ...
  ggplot(aes(fct_rev(education), n)) +
    ...
    geom_text(
      aes(label = glue::glue("{n} times required")),
      family = "Montserrat",
      color = bl_col,
      fontface = "bold",
      hjust = .5,
...
if(save == T){ 
  ggsave(here::here("plots", "5_3_require_tools_ind2.pdf"), 
         width = 12, height = 7, device = cairo_pdf) 
}
```
...
df_salary_cities %>% 
  ggplot(aes(avg, fct_reorder(location, avg))) +
   ...
   scale_fill_manual(
     values = c(darken(bl_col, .5), lighten(bl_col, .7), darken(bl_col, .25), "grey70", bl_col),
     guide = F
   ) +
   labs(
     x = "Average salary",
