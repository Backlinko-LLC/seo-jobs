...
df_salary <- 
  df_gd %>% 
  dplyr::select(job_title, salary, employer, ratings, description, location, url) %>% 
  filter(!is.na(salary)) %>% 
  mutate(
...
"SEO Director" by Etsy in Brooklyn, NY, requires JavaScript, HTML and CSS with an estimated average salary is \$116K (estimated range \$108-123K). The average salary in the state of New York is \$71.6K and \$70K as median salary.
...
"SEO Manager" by Quizlet in San Francisco, CA, with an estimated average salary of $107.5K has listed no degree requirements.
