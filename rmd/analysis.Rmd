...
df_en %>% 
  ...
  ggplot(aes(fct_reorder(word, n), n)) +
    ...
    labs(
      ...
      title = '"MANAGER", "SPECIALIST" AND "MARKETING" ARE THE MOST\nCOMMON WORDS USED IN SEO JOB TITLES.',
...
ggplot(
    ...
    labs(
      ...
      title = '“SENIOR SEO MANAGER”, "REMOTE ROLE POSSIBLE",\n“HEAD OF SEO” AND “SEO CONTENT WRITER” ARE THE\nMOST COMMON PHRASES USED IN SEO JOB TITLES.',
...
if(save == T){
  ggsave(here::here("plots", "1_1_jobs_cat.pdf"), width = 12, height = 13.5, device = cairo_pdf)
}
```
...
df_stack %>% 
  ggplot(aes(1, n)) +
    ...
    labs(title = "ONLY 22.9% OF SEO JOBS ARE HIGHLY TECHNICAL.") +
    theme_void() +
...
df_cities %>% 
  ggplot(aes(n, fct_reorder(location, n))) +
    ...
    labs(
      ...
      y = NULL,
      title = "COMPANIES BASED IN NEW YORK, SAN FRANCISCO, AUSTIN AND CHICAGO\nHAVE THE HIGHEST NUMBER OF JOB OFFERS IN THE SEO INDUSTRY.",
...
df_gd %>% 
  ...
  ggplot(aes(sector, n)) +
    ...
    labs(
      ...
      title = 'COMPANIES IN THE "BUSINESS SERVICES" AND "IT" HIRE SEO\nPROFESSIONALS MOST OFTEN.'
    ) +
    theme_flip +
...
df_gd %>% 
  ...
  ggplot(aes(industry, n)) +
    ...
    labs(
      ...
      title = 'INDUSTRIES WITH THE HIGHEST NUMBER OF SEO JOB POSTINGS INCLUDE\n"ADVERTISING & MARKETING", "STAFFING & OUTSOURCING" AND "IT".',
...
df_size_edu <-
  ...
  mutate(
    ...
    bachelors = if_else(education == "bachelors", 1, 0),
    masters = if_else(education == "masters", 1, 0),
    doctorate = if_else(education == "doctorate", 1, 0)
  ) %>% 
...
df_size_edu %>% 
  ...
  ggplot(aes(fct_rev(education), n)) +
    ...
    labs(
      x = NULL,
      y = NULL,
      title = "35.7% OF SEO POSITIONS REQUIRE A COLLEGE DEGREE."
    ) +
    theme(
...
df_size_edu_non %>% 
  ...
  ggplot(aes(
    ...
    labs(
      x = NULL,
      y = NULL,
      title = "64.3% OF SEO POSITIONS DON’T REQUIRE A COLLEGE DEGREE."
    ) +
    theme(
...
df_prog %>% 
  ...
  ggplot(aes(language, count)) +
    ...
    labs(
      ...
      title = "THE MOST IN-DEMAND PROGRAMMING LANGUAGES IN SEO ARE\nHTML, CSS, JAVASCRIPT, GO, SQL AND PHP."
    )
...
df_prog %>% 
  ...
  ggplot(aes(combination, n)) +
    ...
    labs(
      ...
      title = "EMPLOYERS WANT SEO PROFESSIONALS TO KNOW A COMBINATION\nOF HTML, CSS AND JAVASCRIPT.",
...
df_req_tools %>% 
  ggplot(aes(n_tools, n)) +
    ...
    labs(
      ...
      y = NULL,
      title = "63.4% OF SEO POSITIONS CITE EXPERIENCE WITH A SPECIFIC SEO TOOL."
    )
...
df_en_tools %>% 
  ...
  ggplot(aes(count, tool)) +
    ...
    labs(
      ...
      y = NULL,
      title = "EMPLOYERS WANT SEO CANDIDATES TO KNOW GOOGLE ANALYTICS,\nSEMRUSH, GOOGLE SEARCH CONSOLE, MOZ, SCREAMING FROG AND AHREFS.",
...
if(save == T){ 
  ggsave(here::here("plots", "5_3_require_tools_ind.pdf"), 
         width = 12.3, height = 8.7, device = cairo_pdf) 
}
```
...
df_en %>% 
  ...
  ggplot(aes(exp, n)) + 
    ...
    labs(
      x = "Years of experience required",
      y = "Number of job offers",
      title = "MOST SEO POSITIONS REQUIRE 2–5 YEARS OF EXPERIENCE."
    ) +
    theme(
      panel.grid.major.x = element_blank(),
      panel.grid.minor.x = element_blank()
...
df_salary <- 
  df_gd %>% 
  dplyr::select(job_title, salary, employer, ratings, description, location) %>% 
  filter(!is.na(salary)) %>% 
  mutate(
  ...
  mutate(
    salary_avg = salary_low + ((salary_high - salary_low) / 2),
...
df_salary %>% 
  ggplot(aes(x = salary_avg)) +
    ...
    labs(
      x = "Average salary", 
      y = "Number of job offers",
      title = "THE AVERAGE SEO ANNUAL SALARY IS $60K/YEAR."
    ) +
    theme(panel.grid.minor.x = element_blank())
...
df_salary_cities %>% 
  ggplot(aes(avg, fct_reorder(location, avg))) +
   ...
   geom_text(
     #data = df_salary_cities %>% slice(c(1, 5, 7, 10)),
     data = df_salary_cities %>% slice(c(1, 7, 10)),
     aes(label = state_lump),
     family = "Montserrat",
     fontface = "bold",
     color = "white",
   ...
   scale_x_continuous(
     ...
     labels = glue::glue("${seq(0, 100, by = 20)}K"),
     expand = c(.001, .001),
     limits = c(0, 115)
   ) +
   scale_fill_manual(
     #values = c(darken(bl_col, .5), lighten(bl_col, .7), darken(bl_col, .25), "grey75", bl_col),
     values = c(darken(bl_col, .5), darken(bl_col, .25), "grey75", bl_col),
     guide = F
   ) +
   labs(
     x = "Average salary",
     y = NULL,
     title = "SEO JOBS IN THE SAN FRANCISCO BAY AREA HAVE THE\nHIGHEST AVERAGE SALARIES.",
...
if(save == T){ 
  ggsave(here::here("plots", "6_2_salary_cities.pdf"), 
         width = 12, height = 15.5, device = cairo_pdf) 
}
```
...
df_salary_states %>% 
  ggplot(aes(avg, fct_reorder(state, avg))) +
    ...
    labs(
      x = "Average salary",
      y = NULL,
      title = "SEO JOBS IN CA, CT, NY AND NJ HAVE THE HIGHEST ANNUAL SALARIES.",
...
df_salary_prog %>% 
  ...
  ggplot(aes(prog, avg)) +
    ...
    labs(
      x = "Programming language\nmentioned as requirement?",
      y = "Estimated salary",
      title = "SEO PROFESSIONALS THAT CAN CODE GET ON\nAVERAGE PAID MORE THAN THOSE THAT CAN’T.",
      caption = "The line shows the standard deviation of the average salary per group."
    ) +
    theme(panel.grid.major.x = element_blank(),
  ...
  ggplot(aes(prog, med)) +
   ...
   geom_text(
    ...
    labs(
      x = "Programming language\nmentioned as requirement?",
      y = "Estimated salary",
      title = "MEDIAN INCOME OF SEO PROFESSIONALS THAT\nCODE IS HIGHER THAN FOR THOSE THAT CAN’T.",
      caption = "The line shows the standard deviation of the average salary per group."
    ) +
    theme(panel.grid.major.x = element_blank(),
          plot.title = element_text(size = 18))
...
sample <- 
  df_salary %>% 
  filter(
    str_detect(description, "\\bjavascript\\b"),
    salary_avg > 60
  ) %>% 
 sample_n(1)
```

"Senior Web Engineer - SEO" by StockX in Seattle requires JavaScript, HTML and CSS with an estimated average salary is \$116K (estimated range \$108-123K). The average salary in the state of Washington is \$69.2K and \$64.5K as median salary.
...
"Content & SEO Strategy Manager" by Henkel in Rocky Hill, CT with an estimated average salary of $105K has listed no degree requirements. 
...
df_dates %>% 
  ...
  ggplot(aes(week, n)) +
    ...
    labs(
      ...
      y = "Number of job offers on LinkedIn.com",
      title = "THE COVID-19 PANDEMIC DIDN’T NEGATIVELY IMPACT THE SEO JOB POSTINGS.",
