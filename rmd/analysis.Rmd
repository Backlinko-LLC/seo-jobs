...
if(save == T){ 
  ggsave(here::here("plots", "3_4_rating_best.pdf"), 
         width = 5, height = 2, device = cairo_pdf) 
}
```
...
if(save == T){ 
  ggsave(here::here("plots", "3_4_rating_worst.pdf"), 
         width = 5, height = 1.8, device = cairo_pdf) 
}
```
...
```{r 5-2-requirement-programming-languages, fig.width = 12, fig.height = 7}
df_prog %>% 
...
if(save == T){ 
  ggsave(here::here("plots", "5_3_require_tools.pdf"), 
         width = 12, height = 8, device = cairo_pdf) 
}
```

```{r}
df_en_tools <- colSums(sapply(tools, function(x,y) str_count(y, x), df_en$description ))
...
df_en_tools %>% 
  ...
  ggplot(aes(count, tool)) +
    ...
    labs(
      ...
      y = NULL,
      caption = 'Note: Only tools with a frequency of 5 or more shown.' 
    )
...
```{r 6-2-sdalary-per-city, fig.width = 12, fig.height = 8}
df_salary_cities <-
  df_salary %>% 
  group_by(location) %>% 
  summarize(
...
```{r 6-2-sdalary-per-state, fig.width = 12, fig.height = 8}
df_salary_states <-
  df_salary %>% 
  mutate(
    state = str_extract(location, "[^, ]*$"),
