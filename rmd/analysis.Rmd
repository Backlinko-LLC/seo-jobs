...
```{r 1-1-popular-job-words-eng, fig.width = 12, fig.height = 8}
df_en %>% 
  ...
  ggplot(aes(fct_reorder(word, n), n)) +
    ...
    labs(x = NULL, 
         ...
         caption = 'Note: Only words with a frequency of 500 or more shown. The terms "SEO", "yrs", and "paid" were removed.') +
    theme_flip

if(save == T){
  ggsave(here::here("plots", "1_1_en_jobs_word.pdf"), width = 12, height = 8, device = cairo_pdf)
}
```
...
```{r 1-1-popular-job-words-gd, fig.width = 12, fig.height = 8}
df_gd %>% 
  unnest_tokens(word, job_title, token = "words") %>% 
  anti_join(stop_words) %>% 
  count(word, sort = T) %>% 
...
if(save == T){
  ggsave(here::here("plots", "1_1_gd_jobs_word.pdf"), width = 12, height = 8, device = cairo_pdf)
}
```
...
```{r 1-1-popular-jobs-ngrams-cat-en, fig.width = 12, fig.height = 7.5}
df_job_ngrams_en <-
  df_en %>% 
  ...
  mutate(
    group = case_when(
      ...
      str_detect(word, "developer") ~ "Development",
      str_detect(word, "front end") ~ "Development",
      str_detect(word, "relocate") ~ "Relocation",
      str_detect(word, "relocate") ~ "Relocation",
      str_detect(word, "internship") ~ "Internship",
      TRUE ~ "Other"
    ),
...
ggplot(df_job_ngrams_en, 
    ...
    theme_flip +
    theme(legend.position = c(.63, .558),
          legend.key.size = unit(3, "pt"),
          legend.key.width = unit(30, "pt"))

if(save == T){
  ggsave(here::here("plots", "1_1_en_jobs_cat.pdf"), width = 12, height = 7.5, device = cairo_pdf)
}
```
...
```{r 1-1-popular-jobs-ngrams-cat-gd, fig.width = 12, fig.height = 7.5}
df_job_ngrams_gd <-
  df_gd %>% 
  unnest_tokens(word, job_title, token = "ngrams") %>% 
  anti_join(stop_words) %>% 
...
ggplot(df_job_ngrams_gd, 
    ...
    labs(x = NULL, 
         ...
         caption = "Note: Only sequences with a frequency of 5 or more shown.") +
    theme_flip +
    theme(legend.position = c(.42, .604),
          legend.key.size = unit(3, "pt"),
          legend.key.width = unit(30, "pt"))

if(save == T){
  ggsave(here::here("plots", "1_1_gd_jobs_cat.pdf"), width = 12, height = 7.5, device = cairo_pdf)
}
```
...
```{r 1-1-popular-job-type-en--no-specialist, fig.width = 12, fig.height = 10.5}
...
df_polar %>% 
  ggplot(aes(1, n)) +
    ...
    geom_text(data = df_polar %>% group_by(type) %>% summarize(pos_cont = unique(pos_cont), sum = format(unique(sum), big.mark = ",")),
              ...
              hjust = .5,
              vjust = .5) +
    geom_text(data = df_polar %>% filter(n >= 250),
              aes(1.47, pos, 
                  label = word, 
                  color = type,
                  color = after_scale(colorspace::darken(color, .15)),
...
if(save == T){
  ggsave(here::here("plots", "1_1_en_jobs_tech_adj.pdf"), width = 12, height = 13.5, device = cairo_pdf)
}
```
...
```{r 1-1-popular-job-type-gd, fig.width = 12, fig.height = 12}
...
if(save == T){
  ggsave(here::here("plots", "1_1_gd_jobs_tech.pdf"), width = 12, height = 12, device = cairo_pdf)
}
```

```{r 1-1-popular-job-type-gd--no-specialist, fig.width = 12, fig.height = 10.5}
## technical: analyst, engineer, developer, technician, optimization,
## non-technical: manager, director, writer, consultant, coordinator, editor, marketing, sales, social media 
...
if(save == T){
  ggsave(here::here("plots", "1_1_gd_jobs_tech_adj.pdf"), width = 12, height = 14.5, device = cairo_pdf)
}
```
...
```{r 2-1-map-cities-en, fig.width = 15, fig.height = 9.7}
## add geocodes of cities
path <- here::here("proc_data", "geocodes_all.Rds")

if(file.exists(path)){
  ## load geocodes or...
  df_geo_all <- readRDS(path)
}else{
  ## grab and save geocodes
  df_geo_all <- 
    gio_batch_geocode(df_loc_all$location) %>% 
    unnest(response_results, .preserve = response_warnings) %>% 
    dplyr::select(query, formatted_address, location.lat, location.lng) %>% 
    group_by(query) %>% 
    slice(1)
  
  saveRDS(df_geo_all, path)
}

sf_map_all <-
  df_loc_all %>% 
  left_join(df_geo_all, by = c("location" = "query")) %>% 
...
if(save == T){ 
  ggsave(here::here("plots", "2_1_all_map_cities.pdf"), 
...
sf_map_en %>% 
  ...
  ggplot() +
    ...
    geom_sf(aes(size = n, color = n),
            ...
            alpha = .1,
            show.legend = "point") +
    coord_sf(xlim = c(-4150000, 4050000), 
             ylim = c(-2200000, 3650000)) +
    scale_size(range = c(1, 25), 
               breaks = c(10, 50, 100, 250, 500, 1000),
               name = "Number of job offers") +
    guides(size = guide_legend(title.position = "top", 
                                title.hjust = .5, 
                                nrow = 1,
                                ...
                                override.aes = list(shape = 21, color = bl_col, fill = colorspace::lighten(bl_col, .9), stroke = 1))) +
    theme_map +
    theme(legend.position = c(.19, .12),
          legend.title = element_text(color = "grey80", margin = margin(0, 0, -5, 0)),
          legend.text = element_text(color = "grey60", margin = margin(-5, 0, 0, 0)),
          panel.background = element_rect(fill = "grey30"),
          panel.border = element_rect(color = "grey60", fill = "transparent", size = 3.5),
          panel.grid.major = element_line(color = "grey45", size = .4, linetype = "dashed"),
          plot.margin = margin(0, 0, 0, 0))

if(save == T){ 
  ggsave(here::here("plots", "2_1_en_map_cities.pdf"), 
         width = 15, height = 10.7, device = cairo_pdf) 
}
```
...
```{r 2-1-map-cities-gd, fig.width = 15, fig.height = 9.7}
## add geocodes of cities
path <- here::here("proc_data", "geocodes_gd.Rds")

if(file.exists(path)){
  ## load geocodes or...
  df_geo_gd <- readRDS(path)
}else{
  ## grab and save geocodes
  df_geo_gd <- 
    gio_batch_geocode(df_loc_gd$location) %>% 
    unnest(response_results, .preserve = response_warnings) %>% 
    dplyr::select(query, formatted_address, location.lat, location.lng) %>% 
    group_by(query) %>% 
    slice(1)
  
  saveRDS(df_geo_gd, path)
}

sf_map_gd <-
  df_loc_gd %>% 
  left_join(df_geo_gd, by = c("location" = "query")) %>% 
  filter(!is.na(location.lng)) %>% 
  st_as_sf(coords = c("location.lng", "location.lat"), 
           crs = 4326) %>% 
  st_transform(st_crs(states)) %>% 
...
if(save == T){ 
  ggsave(here::here("plots", "2_1_gd_map_cities.pdf"), 
         width = 15, height = 9.7, device = cairo_pdf) 
}
```
...
if(save == T){ 
  ggsave(here::here("plots", "4_cloud_word2.pdf"), 
